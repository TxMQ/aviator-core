...
public class PlatformLocator {
	...
	/**
	 * When run in test mode, Exo will maintain a single instance of the
	 * application's state so that JUnit tests can run against code that requires
	 * data in the state, without a dependency on the platform being up and running.
	 * 
	...
	/**
	 * Places Exo in test mode, using the passed-in instance of a state. This is
	 ...
	 */
	public static void enableTestMode(Class<? extends AviatorState> stateClass)
	...
	/**
	 * Initializes the platform from an exo-config.json file located in the same
	...
	public static synchronized void initFromConfig(Platform platform) throws ReflectiveOperationException {
		...
		if (AviatorConfig.has("socketMessaging")) {
			try {
				...
				if (messagingConfig.secured == true) {
					initSecuredSocketMessaging(messagingConfig.port, messagingConfig.handlers,
							messagingConfig.clientKeystore.path, messagingConfig.clientKeystore.password,
							messagingConfig.serverKeystore.path, messagingConfig.serverKeystore.password);
				} else {
		...
		// TODO: Refactor all the startup and shutdown activities out of
		// PlatformLocator.
		// It's possible we can eliminate PlatformLocator completely.
		...
		for (Method method : startupMethods) {
			if (Modifier.isStatic(method.getModifiers())) {
				method.invoke(null, new Object[] { platform, testState != null });
			}
		}
	}
	...
	/**
	 * Initializes the platform from an exo-condig.json file located using the path
	 * parameter.
	 * 
	 * @param path
	 * @throws ClassNotFoundException
	...
	public static synchronized void init(Platform platform, List<String> transactionProcessorPackages)
			throws ReflectiveOperationException {
		init(platform);
	...
	public static synchronized void init(Platform platform, List<String> transactionProcessorPackages,
			IBlockLogger logger) throws ReflectiveOperationException {
		init(platform, transactionProcessorPackages);
		blockLogger.addLogger(logger, platform.getAddress().getSelfName());
	}

	/**
	 * Initializes Grizzly-based REST interfaces defined in the included package
	 * list, listening on the included port. Enabling REST will automatically expose
	 * the endpoints service and generate an ANNOUNCE_NODE message.
	 * 
	 * @param port
	 * @param packages
	 */
	...
	/**
	 * Initializes Grizzly-based REST interfaces as defined in the messaging config.
	 * This method will allow for REST endpoints using HTTPS by defining keystore
	 * information in exo-config.json. Enabling REST will automatically expose the
	 * endpoints service and generate an ANNOUNCE_NODE message.
	 * 
	 * @param restConfig
	 */
	public static void initREST(MessagingConfig restConfig) {
		...
		try {
			...
			createTransaction(new AviatorMessage<Serializable>(AviatorCoreTransactionTypes.NAMESPACE,
	 ...
	 * 
	 * @ExoTransaction and automatically maps incoming messages to matching
	 *                 handlers. Any transactions which have not been mapped are
	 *                 passed through the platform to be processed by the model.
	 * 
	 *                 This method creates unsecured sockets with no authentication
	 *                 or in-transit encryption. Beware of dragons.
	 * 
	 * @param port
	 * @param packages
	 */
	...
	/**
	 * Sets up a TLS-encrypted socket-based API for communicating with this Swirld
	 * on the supplied port. X.509 certs are used to authenticate connecting
	 * clients, and vice-versa. Scans the supplied list of packages for methods
	 * annotated with
	 * 
	 * @ExoTransaction and automatically maps incoming messages to matching
	 *                 handlers. Any transactions which have not been mapped are
	 ...
	 * @param packages
	 */
	public static void initSecuredSocketMessaging(int port, String[] packages, String clientKeystorePath,
			String clientKeystorePassword, String serverKeystorePath, String serverKeystorePassword) {
		new TransactionServer(platform, port, packages, clientKeystorePath, clientKeystorePassword, serverKeystorePath,
	 ...
	 * ExoPlatformLocator.init() to intialize the locator before calling
	 * getPlatform()
	 */
	...
	/**
	 * Accessor for Swirlds state. Developers must call ExoPlatformLocator.init() to
	 * initialize the locator before calling getState(), unless running in test
	 * mode.
	 * 
	 * Developers should prefer this method to
	 * ExoPlatformLocator.getPlatform().getState() because this method supports
	 * returning a state in test mode without initializing the platform.
	 */
